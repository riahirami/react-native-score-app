...
import {
  GamePlayerNumberEnum,
  GamePlayerNumberToWinEnum,
  GameTypeEnum
} from '../utils/enum';

import { type Game, type Player } from '../types/interfaces/game';

export interface UseGameActionsReturnType {
...
const useGameActions = (): UseGameActionsReturnType => {
  ...
  const getWinnersPlayers = (players: Player[]): Player[] => {
    if (game.finalScore) {
      const hasPlayerExceededFinalScore = players.some(
        (player) => player.score >= game.finalScore
      );
      ...
      if (game.gameType === GameTypeEnum.CHKOBBA) {
        ...
        if (
          ...
          return players.filter(
            (player) =>
              !lost.some((lostPlayer) => lostPlayer.name === player.name)
          );
        }
      }
    }
  ...
  const ifPlayerOnWinnersPlayers = (player: Player): boolean => {
    return !!winnerPlayers?.some(
      (winnerPlayer) => winnerPlayer.name === player.name
    );
  };
